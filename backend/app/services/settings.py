...
async def get_or_create_auth_settings(db: AsyncSession) -> AuthSettings:
    ...
    settings = AuthSettings(
        ...
        saml_enabled=env_settings.saml_enabled,
        saml_idp_entity_id=env_settings.saml_idp_entity_id,
        saml_idp_sso_url=env_settings.saml_idp_sso_url,
        saml_idp_certificate=env_settings.saml_idp_certificate,
        saml_sp_entity_id=env_settings.saml_sp_entity_id,
    )
    db.add(settings)
    await db.commit()
...
async def update_saml_settings(
    ...
    settings.saml_enabled = enabled
    if idp_entity_id is not None:
        settings.saml_idp_entity_id = idp_entity_id
    if idp_sso_url is not None:
        settings.saml_idp_sso_url = idp_sso_url
    if idp_certificate is not None:
        settings.saml_idp_certificate = idp_certificate
    if sp_entity_id is not None:
        settings.saml_sp_entity_id = sp_entity_id
    if display_name is not None:
        settings.saml_display_name = display_name
    if updated_by:
        settings.updated_by = updated_by

    await db.commit()
    await db.refresh(settings)
    logger.info(f"Updated SAML settings (enabled={enabled}) by {updated_by}")
    return settings
...
async def get_effective_saml_config(db: AsyncSession) -> dict:
    """
    Get effective SAML configuration.

    Database settings take precedence over environment variables.
    """
    db_settings = await get_auth_settings(db)

    if db_settings and (db_settings.saml_idp_entity_id or db_settings.saml_idp_sso_url):
        # Use database settings
        return {
            "enabled": db_settings.saml_enabled,
            "idp_entity_id": db_settings.saml_idp_entity_id,
            "idp_sso_url": db_settings.saml_idp_sso_url,
            "idp_certificate": db_settings.saml_idp_certificate,
            "sp_entity_id": db_settings.saml_sp_entity_id,
            "display_name": db_settings.saml_display_name or "SAML",
        }

    # Fall back to environment variables
    return {
        "enabled": env_settings.saml_configured,
        "idp_entity_id": env_settings.saml_idp_entity_id,
        "idp_sso_url": env_settings.saml_idp_sso_url,
        "idp_certificate": env_settings.saml_idp_certificate,
