...
export function NATGatewayDetailPanel({
  natGateway,
  onClose,
}: NATGatewayDetailPanelProps) {
  return (
        ...
        <div className="space-y-6">
          <section>
            ...
            <div className="divide-y divide-gray-100 rounded-lg border border-gray-200 bg-gray-50 px-3 dark:divide-gray-700 dark:border-gray-700 dark:bg-gray-700">
              <DetailRow
                ...
                value={natGateway.nat_gateway_id}
              />
              <DetailRow label="State" value={natGateway.state} />
              <DetailRow
                label="Connectivity Type"
                value={natGateway.connectivity_type}
              />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="divide-y divide-gray-100 rounded-lg border border-gray-200 bg-gray-50 px-3 dark:divide-gray-700 dark:border-gray-700 dark:bg-gray-700">
              <DetailRow
                label="Allocation ID"
                value={natGateway.allocation_id}
              />
              <DetailRow
                label="Network Interface ID"
                value={natGateway.network_interface_id}
              />
            </div>
          </section>
          ...
          {natGateway.tf_managed && (
            <section>
              ...
              <div className="divide-y divide-gray-100 rounded-lg border border-gray-200 bg-gray-50 px-3 dark:divide-gray-700 dark:border-gray-700 dark:bg-gray-700">
                <DetailRow
                  label="State File"
                  value={natGateway.tf_state_source}
                />
                <DetailRow
                  label="Address"
                  value={
                    <span className="font-mono text-xs">
                      {natGateway.tf_resource_address}
                    </span>
                  }
                />
              </div>
          ...
          <section>
            ...
            <div className="divide-y divide-gray-100 rounded-lg border border-gray-200 bg-gray-50 px-3 dark:divide-gray-700 dark:border-gray-700 dark:bg-gray-700">
              <DetailRow
                label="Last Updated"
                value={formatDateTime(natGateway.updated_at)}
              />
              {natGateway.created_at && (
                <DetailRow
                  label="Created"
                  value={formatDateTime(natGateway.created_at)}
                />
              )}
            </div>
          </section>
