...
import { initiateOIDCLogin, initiateSAMLLogin } from '@/api/client';
...
export function LoginPage() {
  ...
  const handleSAMLLogin = async () => {
    ...
    try {
      ...
      // Store relay state for CSRF verification
      sessionStorage.setItem('saml_relay_state', response.relay_state);
      // Redirect to IdP
      window.location.href = response.sso_url;
    } catch (err) {
      setLocalError('Failed to initiate SAML login');
    }
  };
  ...
  const hasMultipleAuthMethods =
    authConfig &&
    [authConfig.local_auth_enabled, authConfig.oidc_enabled, authConfig.saml_enabled].filter(
      Boolean
    ).length > 1;
